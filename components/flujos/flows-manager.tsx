...
export function FlowsManager({ initialData }: { initialData: Data }) {
  ...
  return <div className="space-y-4">
    ...
    <Card><div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">Fondos virtuales</h2><p className="text-sm text-slate-600">Reservas virtuales: no alteran el saldo ni generan movimientos.</p></div><Button onClick={() => setOpen(!open)} disabled={!initialData.accounts.length}>Asignar dinero</Button></div>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">{initialData.funds.map(fund => <section className="rounded-xl border border-slate-200 p-4" key={fund.id}>
        <div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{fund.name}</h3><p className="text-xs text-slate-500">{fund.currentPeriod?.periodLabel ?? 'Sin periodos'}</p></div><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-medium">{fund.status === 'pendiente_de_iniciar' ? 'Pendiente de iniciar' : fund.status === 'requiere_configuracion' ? 'Requiere configuración' : fund.status.replace('_', ' ')}</span></div>
        <dl className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-3"><div><dt className="text-slate-500">Objetivo del periodo</dt><dd className="font-semibold">{formatCurrencyMXN(fund.periodTarget)}</dd></div><div><dt className="text-slate-500">Pendiente acumulado</dt><dd className="font-semibold">{formatCurrencyMXN(fund.pendingAccumulated)}</dd></div><div><dt className="text-slate-500">Necesidad vigente</dt><dd className="font-semibold">{formatCurrencyMXN(fund.currentNeed)}</dd></div><div><dt className="text-slate-500">Reserva disponible</dt><dd className="font-semibold">{formatCurrencyMXN(fund.availableReserve)}</dd></div><div><dt className="text-slate-500">Necesidad acumulada</dt><dd className="font-semibold">{formatCurrencyMXN(fund.accumulatedNeed)}</dd></div></dl>
      </section>)}</div>
    </Card>
